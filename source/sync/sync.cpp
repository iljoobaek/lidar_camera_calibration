...
int main(int argc, char *argv[])
{
    ...
    {
        std::cout << "Need to have filename in data folder as argument" << std::endl;
        return -1;
    }

    // store timestamps from both vlp-16 and camera
    loadTimestampsIntoVector(source_path[1], &vlp_ts_vec, &vlp_ts);
    loadTimestampsIntoVector(source_path[3], &cam_ts_vec, &cam_ts);
